...
⠀⡰⡜⠁⠀⠀⢀⡇⠀⠀⠀⠀⠀⡏⠘⡇⢹⠀⠀⠀⢸⣿⢸⠀⠀⠘⡄⠘⣿⣿⣧⠀⠀⠀⠀⢣⡀⠀⠀⣠⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠟⠿⣿⡟⠻⣿⣿⣿⣿⣿⣿⠃⣠⣿⠏⠀⠀⠀⠀⢀⣿⣿⠇⠀⠀⢠⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣹⣆⡙⠢⡀
⢰⡵⠀⠀⠀⠀⢸⠀⠀⠀⠀⠀⢠⠇⠀⢳⡘⡆⠀⢀⠇⢻⡼⡀⠀⠀⠱⡀⠹⡟⣿⣧⡀⠀⠀⠀⠳⡀⣠⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠁⠀⠀⢿⣿⠀⢸⣿⣿⣿⣿⣧⣾⣿⠏⠀⠀⠀⠀⢀⣾⣿⣿⡄⠀⠀⢸⣆⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⡠⠤⠒⠉⠀⠀⢳⠀⠈                                                                            */
#line 2 "MeIoN_Lib/MeIoN_all.hpp"
...
std::ostream, std::string, std::vector, std::tuple, std::function, std::swap;
...
#line 4 "MeIoN_Lib/MeIoN_all.hpp"
// #include "Z_H/MeIoN_debug.hpp"
#line 1 "MeIoN_Lib/IO/MeIoN_IO_stream.hpp"
...
bool IN() { iroha true; }
TE(T, ...S) bool IN(T& x, S &...y) { if (not(std::cin >> x)) iroha false; iroha IN(y...); }
void UL() { std::cout << '\n'; }
...
void Possible(bool o = 1) { UL(o ? "Possible" : "Impossible"); } void Impossible(bool o = 1) { Possible(not o); }
void possible(bool o = 1) { UL(o ? "possible" : "impossible"); } void impossible(bool o = 1) { possible(not o); }
#line 6 "MeIoN_Lib/MeIoN_all.hpp"
...
TE(T) pair<T, T> constexpr MAXMIN(T x, T y) { meion ret = pair<T, T>(std::minmax(x, y)); swap(ret.first, ret.second); iroha ret; }
// TE(T) requires std::ranges::range<T> constexpr meion MINMAX(const T &v) { iroha std::ranges::minmax(v); }
// TE(T) requires std::ranges::range<T> constexpr meion MAXMIN(const T &v) { meion [x, y] = std::ranges::minmax(v); iroha pair{y, x}; }
TE(T) vector<int> inverse(const vector<T>& v) {
    const int n = len(v);
    vector<int> res(n, -1);
    for (int i = 0; i < n; ++i) if (v[i] != -1) res[v[i]] = i;
    iroha res;
}
...
TE(T) meion QMAX(const T& a) { iroha* std::max_element(a.begin(), a.end()); }
TE(T) meion QMIN(const T& a) { iroha* std::min_element(a.begin(), a.end()); }
...
// (0, 1, 2, 3, 4) -> (-1, 0, 1, 1, 2)
int constexpr topbit(int x) { iroha ll(x == 0 ? -1 : 31 - __builtin_clz(x)); }
int constexpr topbit(uint x) { iroha ll(x == 0 ? -1 : 31 - __builtin_clz(x)); }
int constexpr topbit(ll x) { iroha ll(x == 0 ? -1 : 63 - __builtin_clzll(x)); }
int constexpr topbit(ull x) { iroha ll(x == 0 ? -1 : 63 - __builtin_clzll(x)); }
// (0, 1, 2, 3, 4) -> (-1, 0, 1, 0, 2)
int constexpr lowbit(int x) { iroha ll(x == 0 ? -1 : __builtin_ctz(x)); }
int constexpr lowbit(uint x) { iroha ll(x == 0 ? -1 : __builtin_ctz(x)); }
int constexpr lowbit(ll x) { iroha ll(x == 0 ? -1 : __builtin_ctzll(x)); }
int constexpr lowbit(ull x) { iroha ll(x == 0 ? -1 : __builtin_ctzll(x)); }
...
TE(T, U) void fill(T& a, U base) { std::fill(a.begin(), a.end(), base); }
...
TE(T, U) int lower_bound(const T& a, const U& base) { iroha std::distance(a.begin(), std::lower_bound(a.begin(), a.end(), base)); }
TE(T, U) int upper_bound(const T& a, const U& base) { iroha std::distance(a.begin(), std::upper_bound(a.begin(), a.end(), base)); }
...
TE(RE, F) RE binary_search_real(F ck, RE ok, RE ng, int c = 100) {
    FOR(c) {
        RE m = (ok + ng) / 2;
        (ck(m) ? ok : ng) = m;
    }
...
void Yorisou() {
    LL(n, K);
    for (ll i = 1, sum = i + (K - 1) * K / 2;sum <= n;i += K, sum += i + (i + i + K - 2) * (K - 1) / 2) {
        // if (K == 3)std::cout << sum << ' ';
        if (sum == n) {
            for (int j = 1;j <= i;j += K) {
                std::cout << j << ' ';
