...
int main() {
    ...
    const int m = n * (n - 1) / 2;
    std::vector<double> ans(m + 1, 1E9);

    auto check = [&](P o, double r) {
        int c = 0;
        for (int i = 0; i < n; i++) {
            if (distance(p[i], o) < r + eps) {
                c++;
            }
        }
        ans[c * (c - 1) / 2] = std::min(ans[c * (c - 1) / 2], r);
        };
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            for (int k = j + 1; k < n; k++) {
                ...
                Point m1 = (p[i] + p[j]) / 2;
                Point v1 = rotate(p[j] - p[i]);
                Point m2 = (p[i] + p[k]) / 2;
                Point v2 = rotate(p[k] - p[i]);
                P o = lineIntersection(Line(m1, m1 + v1), Line(m2, m2 + v2));
                check(o, distance(o, p[i]));
            }
        }
    }
