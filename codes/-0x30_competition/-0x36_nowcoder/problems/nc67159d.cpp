...
Poly<P> power(Poly<P> x, i64 p) {
    Poly<P> ret = { 1 };
    while (p) {
        if (p & 1)ret = ret * x, adjust(ret);
        p >>= 1;
        x = x * x, adjust(x);
    }
    // for (auto a : ret)std::cout << a << ' ';
    return ret;
}

void solve() {
    std::cin >> n >> m >> p;
    std::vector info(n, Poly());
    for (int i = 0;i < n;++i) {
        int k;std::cin >> k;
        info[i] = Poly<P>(11, [&](int i) {return i == 0;});
        for (int j = 0;j < k;++j) {
            int x;std::cin >> x;
            info[i][x] += 1;
        }
