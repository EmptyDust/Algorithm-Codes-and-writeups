...
struct element
{
    ...
    friend constexpr std::ostream& operator<<(std::ostream& os, const element& a) {
        if (a.coefficient == 0)return os;
        return os << a.coefficient << ' ' << a.index << '\n';
    }
    friend constexpr std::istream& operator>>(std::istream& is, element& a) {
        is >> a.coefficient >> a.index;
        return is;
    }
};
