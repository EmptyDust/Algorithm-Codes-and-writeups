...
已按文件名前缀迁移可自动识别来源的历史题目代码，包括 `cf*`、`lc*`、`lgp*`、`ac*`、`lq*`、`nc*`、`abc*`、`sp*`、`lb*`，以及带明确 Codeforces 题号、`cfedu`、`uva`、`lgb/lgt`、`USACO` 标记的文件。

知识点目录中剩余的源码主要是模板草稿、模型代码、课程作业、校内训练或无法从文件名稳定判断来源的历史文件；这些文件不再强行自动迁移。
