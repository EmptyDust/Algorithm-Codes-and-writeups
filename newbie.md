...
3.比赛地点:根据报名情况待定。\
4.比赛规则:比赛采用单人ICPC赛制，允许携带、查阅任意数量的纸质资料，禁止交头接耳，向场外人员求助，禁止使用除了比赛平台和本地编译器之外的任何网页、插件、大语言模型等工具来解决问题，以通过题目数量为第一关键字降序，罚时为第二关键字升序排名。罚时计算规则如下： 
...
$$
\sum_{i=0}^{k-1}a_i + 5 * \sum_{i=0}^{k-1}w_i
$$

题目难度乱序，比赛期间可以实时査看自己和其他选手的答题情况以此来调整自己的答题顺序，比赛最后一小时封榜，只能看到其他选手的提交记录，无法看到其他选手是否通过。两个年级分开评奖。
#### 竞赛奖励
1.奖项及奖品待定。\
